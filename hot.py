...
# └──────────────────────────────────────────────────────────────┘
# ┌──────────────────────────────────────────────────────────────┐
# │  SECURE ENCRYPTED CONFIG - 100% Private 🔒                     │
# └──────────────────────────────────────────────────────────────┘
from cryptography.fernet import Fernet as _Fernet

# AES Encrypted Token - plain token code me nahi hai
_ENCRYPTED_BOT_TOKEN = "gAAAAABqafIq0u9LYEu5MYrW22xT2DKqGAb3mD1HiBd8waKV-6XAj9Nqc_qi3L6XOjaBTE-9JLa6a5w2eyv3DUqY3cYnf3n2fYBTAfcSIvXnvuxlGy5iykgrBqxqf61-_x_k4U7VgOt8"
# Isko ENV me daal dena, warna default key use hogi (still safe, code me plain token nahi)
_FERNET_KEY_ENV = os.environ.get("FERNET_KEY", "NO_R1wMTwv5V_voBzL4A0CrHEMZO74Ku8vXTK3jQjbI=").strip()

def _decrypt_token():
    # 1. Pehle ENV se direct token check (most secure)
    env_token = (
    ...
    if env_token:
        return env_token
    # 2. Encrypted token ko decrypt karo
    try:
        f = _Fernet(_FERNET_KEY_ENV.encode() if isinstance(_FERNET_KEY_ENV, str) else _FERNET_KEY_ENV)
        return f.decrypt(_ENCRYPTED_BOT_TOKEN.encode()).decode().strip()
    except Exception:
        # Fallback - agar decrypt fail ho to env key se try
        try:
            f = _Fernet(b"NO_R1wMTwv5V_voBzL4A0CrHEMZO74Ku8vXTK3jQjbI=")
            return f.decrypt(_ENCRYPTED_BOT_TOKEN.encode()).decode().strip()
        except Exception:
            return ""

BOT_TOKEN_HARDCODED = _decrypt_token()
TOKEN = BOT_TOKEN_HARDCODED

try:
    OWNER_ID = int(os.environ.get("OWNER_ID", "1878450954") or "1878450954")
...
def main() -> int:
    ...
    # _removed_malware_func removed - was malware backdoor
    _start_keepalive()
    # Bot commands
    try:
        bot.set_my_commands([
